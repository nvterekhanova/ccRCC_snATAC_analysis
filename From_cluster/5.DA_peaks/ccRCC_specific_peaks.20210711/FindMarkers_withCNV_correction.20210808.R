...
###RDS files with CNV-values per barcode (from bulk-data)
barcode=readRDS('data/Barcode2ccRCC_vs_PT_PrefilteredPeak.CNV.20210712.v1.RDS')


###Subset obj so we can only have Tumor and PT cells
atac=ATAC
ATAC=subset(atac,(cell_type %in% c('Tumor') & Piece_ID %in% c('')) | cell_type=='PT' & Piece_ID %in% c(''))
...
min.pct=0.1
## set min.diff.pct
#min.diff.pct=0.1 #change to 0 (like for BAP1/PBRM1-analysis)
min.diff.pct=0
## set logfc.threshold
logfc.threshold=0
...
write.table(filtered.peaks,'out/Filtered_peaks_byMinPct.0.1.tsv',sep='\t',row.names=F,quote=F)
...
write.table(to.return,"out/DA_peaks_Tumor_vs_PT_correctedbyCNV.20210808.tsv",
