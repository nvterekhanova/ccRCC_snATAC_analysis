...
ATAC=readRDS(paste('../../../3.Merge_snATAC/Merge.SelectPeaks.v.20210526/',
...
cell_t=read.table('../../../Annotation/26_ccRCC_snATAC_ManualReviwed.v2.20210602.tsv',sep='\t',header=TRUE)
...
####First DOWN-peaks:
peaks=read_delim('data/DOWN_PBRM1_vsNonMutants.Filtered.CNV_corrected.Annotated.20210713.tsv',
delim='\t')
...
y1[,1:(ncol(y1)-2)]=y1[,1:(ncol(y1)-2)]*100000
...
write.table(t1,"out/DOWN_PBRM1mutants_vs_nonMutans.Accessibility.20210713.tsv",sep='\t',quote=F)

----------------------------------
#Now UP peaks:
----------------------------------
peaks=read_delim('data/UP_PBRM1_vsNonMutants.Filtered.CNV_corrected.Annotated.20210713.tsv',
delim='\t')
peaks=as.data.frame(peaks)
...
write.table(t1,"out/UP_PBRM1mutants_vs_nonMutans.Accessibility.20210713.tsv",sep='\t',quote=F)
