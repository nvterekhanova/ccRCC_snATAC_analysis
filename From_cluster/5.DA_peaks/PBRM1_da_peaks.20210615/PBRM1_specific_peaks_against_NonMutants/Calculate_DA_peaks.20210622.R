...
####Only PBRM1-mutants vs non-mutants
...
###We don't include both mutatated!
...
pbrm1_s=meta$Aliquot.WU[meta$Category %in% c('PBRM1-mutant')]
non_pbrm1_s=meta$Aliquot.WU[!(meta$Category %in% c('PBRM1-mutant','Both mutated'))]
non_pbrm1_s=non_pbrm1_s[!(non_pbrm1_s %in% c(''))]
non_pbrm1_s=non_pbrm1_s[!(non_pbrm1_s %in% bap1_s)]
...
all_da_peaks=NULL
for (cell_type in pbrm1_s){
da_peaks <- FindMarkers(
  object = ATAC,
  ident.1 = cell_type,
  ident.2='NOT_PBRM1_mutant',
  only.pos = FALSE,
  min.pct = 0.1,
  min.diff.pct=0,
  logfc.threshold=0,
...
write.table(all_da_peaks, paste("out/da_peaks_PBRM1mut_vs_NonMutants.min.pct0.1.",
