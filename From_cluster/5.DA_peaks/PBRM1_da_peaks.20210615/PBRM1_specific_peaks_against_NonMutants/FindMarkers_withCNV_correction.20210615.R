...
barcode=readRDS('data/Barcode2PBRM1_vs_Nonmutants_PrefilteredPeak.CNV.20210712.v1.RDS')
...
ATAC$test='Other'
ATAC$test=ifelse(ATAC$cell_type=="Tumor" & ATAC$Piece_ID %in% pbrm1_s,"PBRM1_mutants",ATAC$test)
ATAC$test=ifelse(ATAC$cell_type=="Tumor" & ATAC$Piece_ID %in% non_pbrm1_s,"NOT_PBRM1_mutants",ATAC$test)

###Now check how FindMarkers modules will work:
### name it as "object"
object=ATAC
## set ident.use.1
ident.use.1='PBRM1_mutants'
## set ident.use.2
ident.use.2='NOT_PBRM1_mutants'
...
write.table(filtered.peaks,'out/PBRM1_vsNon_mutants.Filtered_peaks_byMinPct.0.1.20210712.tsv',sep='\t',
row.names=F,quote=F)
...
write.table(to.return,"out/DA_peaks_PBRM1mutants_vs_NonMutants_correctedbyCNV.20210713.tsv",
