...
date='20210713'
setwd('/Users/nadezhdaterekhanova/Desktop/Projects/ATAC/CCRCC/Analysis/Updated_pipeline/5.DA_peaks/2021-07-12_BAP1_PBRM1_vs_Nonmutants_28samples/BAP1_vs_NonMutants/')

da_peaks=read_delim('out/da_peaks_BAP1mut_vs_NonMutants.min.pct0.1.20210713.tsv',delim='\t')
...
###cnv_corrected
da_p_c=read_delim('out/DA_peaks_BAP1mutants_vs_nonMutants_correctedbyCNV.20210713.tsv',delim='\t')
...
pdf(paste('UP_BAP1_specific_DA_peaks.After_correction.',date,'.pdf',sep=''),width=8,height=7,useDingbats=F)
barplot(to_plot,ylab='Number of peaks',main='UP peaks after CNV correction',ylim=c(0,300))
dev.off()
...
to_plot=to_plot[order(names(to_plot))]
to_plot=to_plot[order(factor(names(to_plot),levels=c(1:22,"X")))]
pdf(paste('UP_BAP1_specific_DA_peaks.BeforeCNV_correction.',date,'.pdf',sep=''),width=8,height=7,useDingbats=F)
barplot(to_plot,ylab='Number of peaks',main='UP peaks before CNV correction',ylim=c(0,300))
dev.off()
...
#nrow(peaks[peaks$Type=='Promoter',])=158 158/725=21.8%

table(peaks$chr_peak[peaks$Type=='Promoter'])

write.table(peaks, "out/UP_BAP1_vsNonMutants.Filtered.CNV_corrected.Annotated.20210713.tsv",sep='\t',row.names=F,quote=F)

###Fisher.tet:
m=matrix(c(158,567,2907,922),nrow=2)
---------------------------------
###############
#NOW DOWN PEAKS
###############
...
to_plot=to_plot[order(names(to_plot))]
to_plot=to_plot[order(factor(names(to_plot),levels=c(1:22,"X")))]
pdf(paste('DOWN_BAP1_specific_DA_peaks.After_correction.',date,'.pdf',sep=''),width=8,height=7,useDingbats=F)
barplot(to_plot,ylab='Number of peaks',main='UP peaks after CNV correction',ylim=c(0,400))
dev.off()
...
to_plot=to_plot[order(names(to_plot))]
to_plot=to_plot[order(factor(names(to_plot),levels=c(1:22,"X")))]
pdf(paste('DOWN_BAP1_specific_DA_peaks.BeforeCNV_correction.',date,'.pdf',sep=''),width=8,height=7,useDingbats=F)
barplot(to_plot,ylab='Number of peaks',main='UP peaks before CNV correction',ylim=c(0,400))
dev.off()
...
write.table(peaks, "out/DOWN_BAP1_vsNonMutants.Filtered.CNV_corrected.Annotated.20210713.tsv",sep='\t',row.names=F,quote=F)
