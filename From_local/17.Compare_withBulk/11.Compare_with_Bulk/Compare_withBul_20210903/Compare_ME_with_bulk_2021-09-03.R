...
setwd('/Users/nadezhdaterekhanova/Desktop/Projects/ATAC/CCRCC/Analysis/Updated_pipeline/11.Compare_with_Bulk/Compare_withBul_20210903/')
...
####Take from the cell group-motif analysis, but feature TFs, that were identified significant in Tumor vs PT-analysis
...
###Label only TFs found in Tumor-s-PT comparison:
...
####hink about this plot..., maybe plot High in both as well? (epithelial markers)
tumor_specific_tfs= c('RELA','NFKB2','NFKB1','REL','HIF1A','KLF9','RREB1','ARNT::HIF1A','RBPJ','MXI1','ZNF75D','HSF2','SREBF2','NEUROD1','NEUROG2(var.2)','TBXT')
score_sel=score_1[score_1$TF_Name %in% tumor_specific_tfs,]


p <- ggplot(score_1, aes(mean_score_s, Cluster_1_s)) + geom_point(alpha=0.5, colour="grey", data = score_1)

p <- p + geom_point(data = score_sel, colour='blue', alpha=0.95)
...
p <- p + geom_text_repel(data=score_sel,aes(y=Cluster_1_s,x= mean_score_s,label=TF_Name),alpha=1,colour = "black",segment.size=0.3)
